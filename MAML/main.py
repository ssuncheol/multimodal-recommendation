...
def main():
    ...
    train_df, test_df, train_ng_pool, test_negative, num_user, num_item, feature = D.load_data(path,dataset)
    if dataset=='amazon':
        train_dataset = D.CustomDataset_amazon(train_df, feature, negative=train_ng_pool, num_neg=args.num_neg, istrain=True)
        test_dataset = D.CustomDataset_amazon(test_df, feature, negative=test_negative, num_neg=None, istrain=False)
    elif dataset=='movielens':
        train_dataset = D.CustomDataset_movielens(train_df, feature, negative=train_ng_pool, num_neg=args.num_neg, istrain=True)
        test_dataset = D.CustomDataset_movielens(test_df, feature, negative=test_negative, num_neg=None, istrain=False)
