...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, dropRate=0.0, num_classes=1000, zero_init_residual=False,
    ...
    def feature_list(self, x):
        ...
        # out_list.append(out.detach().data.cpu().numpy())
        out_list.append(out)
        out = self.layer1(out)
        # out_list.append(out.detach().data.cpu().numpy())
        out_list.append(out)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        print('Pretrained model is not avalilable.')
    return model
