...
parser.add_argument('--MLP_dim', default='96,128,64,64,32,32', type=str, # 첫번째는 상관없음.
                    help='MLP Dimension')
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch == 0:
            ...
            if dist.get_rank() == 0:
                test(model=model, model_type=args.model, test_loader=test_loader, test_logger=test_logger, epoch=epoch, 
                    test_pos_item_num=test_pos_item_num, item_num_dict=item_num_dict, experiment=experiment, hier_attention=args.hier_attention)
...
def test(model, model_type, test_loader, test_logger, epoch, test_pos_item_num, item_num_dict, **kwargs):
    ...
    print('남은 스코어 개수', score_cat.shape)
